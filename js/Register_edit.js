...
App.controller('mainController', function ($scope, $http) {
    ...
    function checkPageTab(valPage, cEvent) {
        ...
        if (idAreaActive2 === 'tabObject') {
            ...
            showListDataFile();
            tdata = localStorage.getItem('assetData');
            localStorage.clear();
            localStorage.setItem('assetData', tdata);
    ...
    function editAsset() {
        $http.get('./SetListAsset').success(function (data) {
            //$scope.listDataFile = data;
            localStorage.setItem('assetData2', JSON.stringify(data));
    ...
    //////////////////////////////////////////////////////////////get local storage
    function setAttr() {
        if (localStorage.getItem('assetData2') !== null) {
            $('.ng-invalid').removeClass('ng-invalid');
            /* ถ้ามีแล้ว ให้กรอกข้อมูลลงในฟอร์ม โดยใช้ค่าจาก localStorage */
            str = localStorage.getItem('assetData2');
            console.log(str.substring(1, str.length - 1));

            $scope.st = JSON.parse(str.substring(1, str.length - 1));
